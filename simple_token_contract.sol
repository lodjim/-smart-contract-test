...
contract SimpleToken {
    ...
    function mint(address to, uint256 amount) external onlyPositive(amount) {
        balances[to] += amount;
        emit Mint(to, amount);
    }

    function burn(address from, uint256 amount) external onlyPositive(amount) onlyEnough(from, amount) {
